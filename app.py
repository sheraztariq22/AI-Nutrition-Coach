...
def input_image_setup(uploaded_file):
    """
    Processes the uploaded image file to be used with Google Gemini.

    Parameters:
    - uploaded_file: File-like object uploaded via Flask
...
def generate_model_response(image, user_query, assistant_prompt):
    """
    Sends an image and a query to Google Gemini and retrieves the description or answer.
    Formats the response using HTML elements for better presentation.
    """
    try:
        # Combine the assistant prompt with user query
...
def index():
    if request.method == "POST":
        ...
        user_query = request.form.get("user_query", "Please analyze this food image.")
...
"""
                ...
                response = generate_model_response(image, user_query, assistant_prompt)
